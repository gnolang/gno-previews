...
closes; `reindex.yml` here rebuilds the landing page, and `evict.yml` sweeps up whatever
the cleanup misses.

`gnolang/gno` runs **no code from this repository**. The index is regenerated by this
repository's own workflow, with its own token, precisely so that a privileged job over
there never executes a script from an unprotected branch over here.

## A note on the origin

This site shares the `gnolang.github.io` origin with every other GitHub Pages site in the
organisation, so a preview is same-origin with them for cookies and storage purposes. The
snapshots are static and carry no credentials, and every page is `noindex`, but it is the
reason previews are not given a hostname that suggests they are production.
